#!/usr/bin/env python3
"""Export manuscript-v3 tables mechanically from the locked corrective run.
...
MANUSCRIPT_DIRNAME = "manuscript_molecular_diversity_v3_20260728"
...
def validate_lock_and_execution_chain(
    ...
    require_link("component_manifest_to_v1_4", component_manifest["implementation_lock"]["sha256"], sha256_file(paths["document_component_lock"]))
    ...
    require_link("protocol_receipt_to_code_lock", protocol_receipt["code_lock"]["sha256"], sha256_file(paths["implementation_lock"]))
    ...
    evaluation_code_links = (
        ...
        ("run_author_run_asof_cutoff_successor.py", "run_author_script", "scripts/run_author_run_asof_cutoff_successor.py"),
    ...
    for basename, script_name, lock_key in evaluation_code_links:
        ...
        require_link(f"evaluation_{basename}_to_base_lock", implementation_base["source_sha256"][lock_key], actual_script_sha)
    ...
    for basename, script_name, lock_key in scoring_code_links:
        ...
        require_link(f"score_{basename}_to_base_lock", implementation_base["source_sha256"][lock_key], actual_script_sha)
    ...
    for label, runtime, expected_audit, expected_success in (
        ("component_runtime", component_runtime, "document_component_bootstrap_v1_4", True),
...
def build_tables(project_root: Path) -> dict[str, Any]:
    ...
    manifest_path = output_dir / "corrective_table_export_manifest_v3.json"
    manifest = {
        "schema_version": "1.0",
